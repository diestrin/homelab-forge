...
of the Phase 5 release line currently shown on https://localpower.diegobarahona.com.

## How

1. After Slack approval, a Cursor SDK worker claims TASK-006 on branch
...
2. Edit `index.html` inside the `hello-html` ConfigMap — keep valid HTML; include the
   phrase `Forge Software Factory` in visible body copy (exact wording can be refined
   in the Slack thread before approve).
3. Open/update the plan PR; complete [`factory/review/CHECKLIST.md`](../review/CHECKLIST.md)
   before merge.
...
This plan PR is a draft. Reply in the originating Slack thread to adjust copy or AC;
orchestrator updates this file and the task YAML until you approve (`approve` / `lgtm`).
